...
const ReviewCableTV = () => {
  ...
  const biller = params.biller || 'DSTV';
  const billPlan = params.billPlan || 'DSTV Yanga';
  const decoderNumber = params.decoderNumber || '';
  const amount = params.amount || '3500.00';
  const nameOnDecoder = 'ADEMOLA ADENIYI'; // This would typically come from API
  ...
  const transactionRows = [
    { label: 'Biller type', value: biller },
    { label: 'Bill plan', value: billPlan },
    { label: 'Decoder number', value: decoderNumber },
    { label: 'Name on decoder', value: nameOnDecoder },
    { label: 'Amount', value: `NGN${parseFloat(amount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` },
  ];
